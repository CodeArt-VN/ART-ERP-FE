...
export class SaleOrderNoteComponent extends PageBase {
    ...
    loadDeliveryNote() {
        ...
        this.item.OriginalTotalAfterDiscountFromSalesmanText = lib.currencyFormat(this.item.OriginalTotalAfterTax - this.item.OriginalDiscountFromSalesman);
    ...
    generatePaymentQRCODE(SOId: number, Amount: number = 0) {
        ...
        QRCode.toDataURL(bankQRContent, { errorCorrectionLevel: 'M', version: 6, width: 500, scale: 20, type: 'image/webp' }, function (err, url) {
            that.item.QRC = url;
        })
    }
