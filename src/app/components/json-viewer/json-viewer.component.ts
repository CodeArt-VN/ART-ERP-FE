...
export class JsonViewerComponent implements OnInit {
    ...
    Object.keys(this.item).forEach(k => {
      ...
      if (this.item[k] instanceof Array) {
        ...
        this.item[k].forEach(i => {
          if (this.oldItem && this.oldItem[k] && this.oldItem[k][index])
            this.buildDataArray(i, data.Id, index, this.oldItem[k][index]);
          else
            this.buildDataArray(i, data.Id, index, null);
          index++;
        })
      }
    });
  ...
  recurDifference() {
    this.dataSourceState = [...this.dataSourceState.filter(d => d.Value != d.OldValue || (d.Value == d.OldValue &&
      this.dataSourceState.some(f => f.IDParent == d.Id))
    )];
    if(this.dataSourceState.some(d =>(d.Value == d.OldValue &&
      !this.dataSourceState.some(f => f.IDParent == d.Id)))){
        this.recurDifference();
      }
  }
