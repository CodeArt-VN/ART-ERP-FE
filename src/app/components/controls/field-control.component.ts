	...
	template: `
		<div class="c-control" *ngIf="field.data">
			<ion-item class="ion-no-padding ion-no-margin" lines="none" [disabled]="field.disabled" *ngIf="!(field.type == 'toggle')">
				<ion-label class="ion-text-wrap ">
					<label class=" c-label" [attr.for]="field.labelForId">
						{{ field.label }}
						<a (click)="reset(field)" class="clickable danger" *ngIf="field.data.ValueObject != null && !field.data._InheritedConfig">[reset]</a>
						<a (click)="field.showInherited = !field.showInherited" class="clickable success" *ngIf="field.data._InheritedConfig">[Inherit]</a>
					</label>
			...
			<ng-container [ngSwitch]="field.type">
				<ng-select
					*ngSwitchCase="'select-staff'"
					[multiple]="field.multiple"
					[items]="field.DataSource | async"
					[typeahead]="field.SearchInput"
					[loading]="field.SearchLoading"
					[virtualScroll]="true"
					(change)="trackChange(field.data)"
					[(ngModel)]="field.data.ValueObject"
				...
				>
					...
					<ng-template ng-option-tmp let-i="item" let-a="item.Address" let-search="searchTerm">
						<div *ngIf="i">
							...
							<small *ngIf="a">
								<b *ngIf="i.Code"
									><span class="important" [ngOptionHighlight]="search">{{ i.Code }}</span></b
								>
								<span *ngIf="a.AddressLine1"> | {{ a.AddressLine1 }}</span>
								<span *ngIf="a.Ward">, {{ a.Ward }}</span>
								<span *ngIf="a.District">, {{ a.District }}</span>
				...
				>
					<ng-template ng-option-tmp let-i="item" let-search="searchTerm">
						<div *ngIf="i">
							<div>
								<span *ngFor="let l of i.levels">&nbsp;&nbsp;&nbsp;</span>
								<ion-text [color]="i.Color || 'dark'" [ngOptionHighlight]="search">{{ i[field.bindLabel ? field.bindLabel : 'Name'] }}</ion-text>
							</div>
						</div>
					</ng-template>
				</ng-select>
			...
			<div *ngIf="field.showInherited && field.data._InheritedConfig" style="opacity: 0.8; margin: 5px 0 32px 0px;">
				<label class="c-label"> Inherit from </label>
				<app-branch-breadcrumbs
					[maxItems]="3"
					[Id]="field.data._InheritedConfig.IDBranch"
					[Items]="field.data._InheritedConfig._Branches"
