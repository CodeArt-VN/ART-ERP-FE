import { Component, OnInit, Input, Output, EventEmitter, ViewChild, SimpleChanges, ChangeDetectorRef } from '@angular/core';
...
export class CoordinatePickerComponent implements OnInit {
	...
	findCoordinate() {
		...
		let addressTerm = (add.AddressLine2 + ', ' || '') + (add.AddressLine1 + ', ' || '') + (add.Ward + ', ' || '') + (add.District + ', ' || '') + (add.Province || '');
		addressTerm = addressTerm.replace(/ /g, '+').replace(/\//g, '%2F');

		this.httpClient.get('https://maps.google.com/maps/api/geocode/json?address=' + addressTerm + '&sensor=false&key=AIzaSyAtyM-Th784YwQUTquYa0WlFIj8C6RB2uM').toPromise().then((data: any) => {
