	...
	ngOnChanges(changes: SimpleChanges) {
		...
		if (this.pageConfig.pageName == 'sale-order') {
			...
			this.selectedItems.forEach((i) => {
				...
				if (
					notShowSubmitOrdersForApproval.indexOf(i._Status.IDStatus) >
					-1
				) {
				...
				let notShowApproveOrders = [
					101, 102, 104, 105, 106, 107, 108, 109, 111, 112, 113, 114,
					115,
				];
				...
				let notShowDisapproveOrders = [
					101, 102, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114,
					115,
				];
		...
		if (
			...
			this.selectedItems.forEach((i) => {
				...
				if (
					['Submitted', 'Approved', 'Cancelled'].indexOf(i.Status) >
					-1
				)
					this.showSubmit = false;
				if (
					['New', 'Unapproved', 'Approved', 'Cancelled'].indexOf(
						i.Status
					) > -1
				)
					this.showApprove = false;
				if (['New', 'Unapproved', 'Cancelled'].indexOf(i.Status) > -1)
					this.showDisapprove = false;
				if (
					['Submitted', 'Approved', 'Cancelled'].indexOf(i.Status) >
					-1
		...
		if (this.pageConfig.pageName == 'request') {
			...
			this.selectedItems.forEach((i) => {
				...
				if (
					notShowSubmitOrdersForApproval.indexOf(i.Status) > -1 ||
					!i.canSubmit
				) {
					this.showSubmit = false;
				}
