	...
	renderURL() {
		if (this.formGroup.get('URL').value) {
			let url = this.formGroup
				.get('_Params')
				.value?.filter((param) => param.Value !== undefined && param.Value !== null && param.Value !== '' && param.Disabled != true)
	...
	async export() {
		...
		this.pageProvider.commonService
			...
			.then((response: any) => {
				...
				let date = new Date();
				let filename = date.getDate() + '-' + (date.getMonth() + 1) + '-' + date.getFullYear() + ' -' + this.formGroup.get('Name').value;
		...
		this.env.getStorage('Variables').then((result) => {
			this.env
				.showLoading(
					...
					this.pageProvider.commonService.connect('POST', 'SYS/APICollection/Run', { Id: this.item.Id, Variables: result }).toPromise()
				)
				.then((res: any) => {
