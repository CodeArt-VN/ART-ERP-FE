...
import { BRA_BranchProvider, SYS_ActionProvider, SYS_IntegrationProviderProvider, SYS_TriggerActionProvider, SYS_TriggerProvider } from 'src/app/services/static/services.service';
...
export class IntegrationTriggerDetailPage extends PageBase {
	...
	preLoadData(event?: any): void {
		Promise.all([this.integrationProvider.read({ IsTriggerable: true }, false), this.env.getType('IntegrationTriggerType'), this.actionProvider.read(this.query, false)]).then(
			(values: any) => {
	...
	runTrigger() {
		this.env
			.showLoading('Please wait for a few moments', this.pageProvider.commonService.connect('POST', 'SYS/Trigger/Run', { Id: this.item.Id }).toPromise())
			.then((_) => {
